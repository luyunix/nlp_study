...
## 真正看懂本节：句长变化时哪一维必须跟着变

固定 `B=2、D=3、H=5、layers=1`，只把输入从 4 个时间步改成 7 个：

```text
x₁ [2,4,3] → output₁ [2,4,5]，h₁ [1,2,5]
x₂ [2,7,3] → output₂ [2,7,5]，h₂ [1,2,5]
```

只有 output 的时间维 L 从 4 变 7；最后状态仍是每条序列一份 H 维记忆，所以形状不包含 L。数值当然会变化，因为模型多读了三个输入。

这不代表一个 batch 可以直接放不同长度的普通张量。组成 `[B,L,D]` 前仍要 padding 到统一 L，或使用 packed sequence。若取 `output[:,-1,:]`，padding 后最后位置可能只是 PAD；应使用真实长度定位最后有效状态，或用 packing/mask。

句长 L 影响运行的时间和中间激活量，不改变 RNN 权重矩阵尺寸。参数由 D、H 和层数决定，所以同一模型可以处理不同 L。
