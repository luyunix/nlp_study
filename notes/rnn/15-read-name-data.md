...
## 真正看懂本节：读文件最怕“看似读到了，实际对应错了”

每个类别文件通常一行一个姓名。读取后理想结构是：

```text
category_lines["Italian"] = ["Abate", "Abbiati", ...]
```

文件路径提供类别，文件内容提供样本。必须检查：文件数是否等于预期类别数、空行是否过滤、每类实际读到多少条、规范化后是否出现空字符串或重复姓名。

Unicode 规范化的目标是让字符落入课程字符表，例如去掉某些组合音标；它可能损失原始拼写，所以应同时保留 raw 与 normalized。不能先规范化后再也找不回原文。

随机抽样时要区分“为了展示取一个样本”和“正式划分训练/验证/测试”。同一个姓名或规范化后相同的变体不能跨集合，否则模型会在测试时见过答案。若数据按来源文件存在批次规律，还要避免仅做逐行随机导致来源泄漏。

读入成功的最低证据不是打印一条姓名，而是一张每类计数表、若干规范化前后对照和重复/空值统计。

## 零基础精讲：先把这一节真正弄懂

### 先用一个场景理解

CSV 中可能有空行、额外逗号或无法解析的字符。读数据不是“文件能打开”就完成，而是要把异常行变成可追踪的决定。
...
先打印前几条 name,country 和异常计数，再确认标签是否都能映射到国家表。
