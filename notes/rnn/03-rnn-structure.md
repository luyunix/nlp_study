...
```text
hₜ = tanh(xₜ Wₓₕᵀ + hₜ₋₁ Wₕₕᵀ + b)
```

设 `xₜ:[B,D]`、`hₜ₋₁:[B,H]`，为了让两项都得到 `[B,H]`：
...
相加并经过 tanh 后，新状态仍为 `[B,H]`，可以继续传给下一时间步。所有 t 共用同一组 `Wₓₕ、Wₕₕ`；若每个位置都有独立权重，模型无法把在第 1 个位置学到的规律复用到第 10 个位置，参数量也会随句长变化。

展开图中画出多个 RNN 方框，只是把同一个单元在时间上的重复计算画开，不代表创建了多个互不相关的层。`num_layers=2` 才表示纵向堆叠两层：第一层每步输出作为第二层同一步输入。

状态能否真正保留长距离信息不只由 H 决定。连续乘递归权重和激活导数会让梯度过小或过大，这就是后面门控结构和梯度裁剪出现的动机。
