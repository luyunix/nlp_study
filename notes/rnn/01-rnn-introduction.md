...
## 真正看懂本节：RNN 比普通网络多出的只有“上一刻”

读句子 `这家酒店不差` 时，如果模型只看当前词“差”，容易判断为负面；人会把前面的“不”带到当前位置。RNN 用隐藏状态 `hₜ` 保存这种随时间更新的信息：

```text
hₜ = f(xₜ, hₜ₋₁)
```

`xₜ` 是当前 token 的向量，`hₜ₋₁` 是读完前一个 token 后的记忆，`hₜ` 是把二者合成后的新记忆。序列不是一次塞进四套不同网络，而是**同一个 RNN 单元重复使用同一组参数**：
...
若输入形状为 `[B,L,D]`，普通单向 RNN 会输出每个时间步的表示 `[B,L,H]`，并额外返回最后状态 `[layers,B,H]`。`H` 是记忆宽度，不等于模型能记住 H 个词。

RNN 的局限也从这条链产生：后面的计算必须等待前一步，难以跨时间完全并行；很早的信息要经过许多次更新，梯度可能衰减或爆炸。LSTM/GRU 用门控改善长期依赖，但没有取消逐步递归。

## 零基础精讲：先把这一节真正弄懂

### 先用一个场景理解

读“这部电影不太好看”时，看到“好看”不能忘记前面的“不”。RNN 的隐藏状态就是一张随阅读不断改写的临时便签。
...
先只手推一个时间步：当前词向量 x_t 和旧便签 h_(t-1) 进入同一单元，得到新便签 h_t。
