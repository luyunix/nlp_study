...
## 真正看懂本节：三种模型应共享任务头，只替换序列编码器

姓名分类的数据输入和输出目标没有变：

```text
x [B,L,57] → sequence encoder → sentence [B,H] → Linear(H,18)
```

变化只在 encoder：

- RNN：返回 `output,h_n`；
- GRU：返回 `output,h_n`，但内部有门控；
- LSTM：返回 `output,(h_n,c_n)`。

把共同的 Linear、类别映射和预处理复制成三份，会让比较时不小心改出差异。更稳妥是同一个分类器接受 `kind`，只在状态解包处分支。

公平比较必须明确 H 的含义相同，但参数量不会相同：LSTM 通常有四组门计算，GRU 三组，普通 RNN 更少。相同 hidden_size 不代表相同参数预算。可分别报告参数量、时间与验证指标，而不是把速度差全部归因于模型名称。

checkpoint 文件要记录 kind、D、H、层数、方向和类别映射；仅凭 state_dict 无法安全重建错误结构。
