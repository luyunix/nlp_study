...
## 真正看懂本节：测试要证明“语义形状”，不只是没有报错

给 `B=1、L=6、D=57、H=128、C=18` 的输入，至少要断言：
...
还要检查 `log_probs.exp().sum(dim=-1)≈1`、没有 NaN/Inf、argmax 类别落在 `[0,18)`。若使用随机输入和随机权重，不能断言具体国家，只能检查结构性质。

真实字符样本比纯随机张量多验证一层：字符编码是否与 D 对齐、序列轴方向是否正确。最常见的“能运行但错了”是把 `[L,1,D]` 当 `[1,L,D]`、误把 hidden 128 当类别输出，或在错误维度做 argmax。

测试时 `model.eval()` 关闭训练态行为，`torch.no_grad()` 避免构建梯度图；两者职责不同。单元测试不需要长时间训练，先把接口错误挡在训练前。
