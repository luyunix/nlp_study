...
## 真正看懂本节：公平测试要固定除模型以外的所有变量

同一条姓名张量 `[1,L,57]` 依次送入 RNN、LSTM、GRU，三个分类器都应输出 `[1,18]`。外部接口一致，内部状态结构不同：
...
若为了让某模型跑通而改变字符表、类别顺序、训练样本或 final state 读取方式，之后的指标就不能直接比较。测试夹具应复用同一个 x、相同 H/层数/方向和相同分类头约定。
...
并检查每个模型对变长输入、batch、device 的行为。相同 H 的 LSTM 参数更多是结构事实，不是实现错误。

随机前向不能判断准确率，但能尽早发现 LSTM 状态解包、方向维、Linear 输入宽度和类别维错误。所有结构测试通过后，才值得启动训练。
