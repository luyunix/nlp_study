...
- loss 曲线：优化是否在训练目标上前进，是否震荡或过拟合；
- accuracy/F1 曲线：任务预测是否改善，少数类是否被照顾；
- 时间/吞吐：取得该效果需要多少计算。

比较曲线时横轴必须同口径。按 epoch 画图会忽略不同模型每轮耗时；按 wall-clock time 则更适合看“同样时间预算谁更好”。训练 loss 与验证指标要分开画，不能把训练集更低的 loss 当成泛化更好。

若 LSTM 最终准确率高 0.3 个百分点但训练时间翻倍，要结合业务价值、波动和置信度判断。一次运行的线条不包含随机性，最好汇总多个种子或至少说明这只是课程演示。

类别不平衡的 18 类姓名任务不能只看总体 accuracy；应补宏平均 F1、每类召回和混淆矩阵，确认模型没有只偏向样本多的类别。

图表是实验记录的视图，源数据、参数和划分也要保存；只有图片无法复核。
