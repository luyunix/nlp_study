...
## 真正看懂本节：hidden_size 改的是表示宽度和参数量

设输入特征 `D=3`。当 `H=5` 时：

```text
W_ih [5,3]，W_hh [5,5]，output [...,5]
```

改为 `H=8`：

```text
W_ih [8,3]，W_hh [8,8]，output [...,8]
```

循环权重包含 `H×H` 项，所以 H 增大时参数和计算并非只线性增加。PyTorch 的普通 RNN 还包含偏置；双向和多层会按方向、层数继续增加。

H 可以理解为每个时间步用多少个数记录上下文，容量更大可能表达更多模式，也可能过拟合、变慢和占用更多显存。它不等于“记住多少个词”，也不保证解决长距离梯度问题。

修改 H 后，下游线性层也要同步。例如原来 `Linear(5,C)` 必须改成 `Linear(8,C)`；加载旧 checkpoint 时权重形状不匹配，不能强行复用。
