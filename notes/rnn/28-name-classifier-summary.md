# 第 28 节：姓名分类总结：按单模型跑通，再统一重构

> 笔记编号 28/28 · 对应原视频 P65 · [打开这一集](https://www.bilibili.com/video/BV14mdfBDE4Q?p=65)

[← 上一节：27 RNN 预测：姓名转张量、加载权重并取 Top-k](./27-rnn-prediction.md) · [返回总目录](./README.md) · 已是最后一节 →

## 这节解决什么问题

面对数百行项目代码，零基础应按什么顺序练习才不会迷失？

![第 28 节原创概念图](./diagrams/28-concept.svg)

图从左向右读。先跟着数据或推理过程走一遍，再学习下面的术语。

## 辅助流程图

```mermaid
flowchart LR
    N0["先只跑 RNN"]
    N1["数据→模型→训练→预测"]
    N2["再独立迁移 LSTM"]
    N3["再迁移 GRU"]
    N4["最后统一重构与优化"]
    N0 --> N1
    N1 --> N2
    N2 --> N3
    N3 --> N4
```

### 姓名分类项目完整流水线

```mermaid
flowchart LR
    A["CSV：name,country"] --> B["字符规范化"]
    B --> C["每字符 57 维 One-Hot"]
    C --> D["Dataset"]
    D --> E["DataLoader"]
    E --> F["RNN / LSTM / GRU"]
    F --> G["18 类 log-probabilities"]
    G --> H["NLLLoss + 反向传播"]
    H --> I["保存权重 / 比较曲线 / Top-k 预测"]
```



## 真正看懂本节：用一张验收表复盘整个项目

| 阶段 | 必须保存/检查 | 常见错误 |
|---|---|---|
| 数据 | 原文、类别、划分、每类数量 | 同名样本跨训练与测试 |
| 编码 | 字符表、规范化、UNK 规则 | 训练预测字符 ID 不一致 |
| Batch | x/lengths/mask/y 的形状 | PAD 被当真实字符 |
| 模型 | kind、D、H、层数、方向、C | LSTM 的 h/c 解包错误 |
| 训练 | loss 配套、优化器、最佳验证指标 | 用训练准确率选模型 |
| 预测 | eval/no_grad、top-k、类别映射 | 把类别 ID 当业务标签 |

真正完成不是三个模型都能打印 `[1,18]`，而是同一数据管道下可复现地训练、验证、加载和预测。建议先让普通 RNN 端到端跑通，再把 encoder 换为 LSTM/GRU；一次只改一个组件，问题更容易定位。

模型对比要同时看参数量、时间、宏平均指标和每类错误。课程小数据上的胜负不具有普遍结论。

最后牢记边界：这个项目演示字符序列分类，不应把姓名预测结果当成个人真实国籍，更不应用于高风险身份决策。技术流水线可迁移，业务结论必须重新验证。

## 零基础精讲：先把这一节真正弄懂

### 先用一个场景理解

面对完整项目，先让 RNN 从数据到预测闭环，再逐个替换 LSTM、GRU。一次只改变一处，错误才有清楚来源。

### 沿数据流一步一步走

1. 先只跑 RNN
2. 数据→模型→训练→预测
3. 再独立迁移 LSTM
4. 再迁移 GRU
5. 最后统一重构与优化

上面每一步都对应流程图的一段。读图时不断问自己：“此刻张量里装的是什么，形状是什么，下一步为什么需要它？”

### 第一次看代码只盯住这里

把项目拆成数据、模型、训练、预测四个可单测模块；最后才抽象公共代码和画对比图。

运行代码前先写出预期形状，运行后逐维核对。数值可以暂时算不出，但 B（批量）、L（长度）、D/H（特征或隐藏宽度）为什么出现，必须能说清。

### 本节边界

一开始把三模型、绘图、优化全部混在一起，会让任何错误都难以定位。

本节过关不是背公式，而是能从第 1 步讲到最后一步，并指出哪一个状态把前文带到了后面。

## 老师原声整理稿（按讲解顺序）

### 0:00–1:52　不要三模型同时抄

老师建议先忽略 LSTM/GRU，只用 RNN 从数据处理、建模、训练到预测完整跑一遍。先建立一条能闭环的主线。

### 1:52–3:47　再分别迁移

RNN 熟练后，单独建立 LSTM 版本，再建立 GRU 版本；最后才把三者合并成课堂的统一对比版本。这样每次只改变一个模型因素，错误更好定位。

### 3:48–4:59　优化清单

老师要求把可优化点真正落地：使用 CrossEntropyLoss 简化输出、直接返回 DataLoader、把损失/准确率/耗时持久化后再绘图，并改进其他重复代码。绘图函数可复用，但数据、模型、训练和预测应自己写。

## 完整原声逐段记录

[查看本节按时间戳整理的完整音轨转写](./transcripts/p065.md)

逐段记录用于核查老师讲解是否遗漏；正文会进一步纠正口误和语音识别中的技术术语。

## 零基础先记住

- 先单模型闭环，再做抽象
- 每次迁移只改必要部分
- 优化应伴随测试而不是只改写法

## 最小可运行代码

下面代码默认从项目根目录运行；专题配套实现见 [rnn_from_scratch 配套实现](../../rnn_from_scratch/README.md)。

```python
stages = ["RNN 闭环", "LSTM 迁移", "GRU 迁移", "统一接口", "测试与持久化"]
for i,s in enumerate(stages,1): print(i,s)
```

### 输入和输出怎么看

按难度递增打印五个练习阶段。

## 最容易踩的坑

一开始把三模型、绘图、优化全部混在一起，会让任何错误都难以定位。

## 本节知识链

`先只跑 RNN → 数据→模型→训练→预测 → 再独立迁移 LSTM → 再迁移 GRU → 最后统一重构与优化`

## 自测

**问题：最适合零基础的第一步是什么？**

<details>
<summary>点开核对答案</summary>

只用 RNN 完整跑通数据处理→模型→训练→预测。

</details>

## 学完检查

- [ ] 我能用自己的话复述老师的讲解顺序
- [ ] 我能在运行前预测关键输出或张量形状
- [ ] 我知道这节方法最容易用错的地方
- [ ] 我能独立回答自测题

[← 上一节：27 RNN 预测：姓名转张量、加载权重并取 Top-k](./27-rnn-prediction.md) · [返回总目录](./README.md) · 已是最后一节 →
