...
句子 `猫 爱吃 鱼` 中若 `爱吃` 不在这个词表，先不要偷偷添加新列；假设本例只编码已知序列 `猫 鱼 猫`，每个位置依次得到：
...
形状是 `[L,V]=[3,3]`：行数来自句子 token 数，列数来自固定词表大小。重复词的行必须完全相同。

### 0-based 和 1-based 为什么会错一列

Python 列表下标从 0 开始。有些 tokenizer 把 0 留给 padding，让第一个真实词 ID 从 1 开始。如果词 `猫` 的 ID 是 1，却直接执行 `vector[1]=1`，1 就落在数组第二列。解决方式只能二选一并保持全程一致：

- 建立 0-based 词表，ID 可直接当数组下标；
- 保留 1-based ID，并让向量额外包含保留位或明确减 1。

不能在训练时用一种约定、预测时换另一种。

### One-Hot 矩阵有哪些可检查的不变量

在没有 OOV、多标签和 padding 的最简单词级编码中：

1. 矩阵形状必须是 `[token 数, 词表大小]`；
2. 每行元素只能是 0 或 1；
3. 每行的和恰好为 1；
4. 相同 token 对应相同行；
5. 每一列的语义由保存的词表永久决定。

这些检查比观察控制台一大串 0 更可靠。若未知词允许映射到 `<UNK>`，它也应落在固定 UNK 列，而不是产生全零行；全零容易与 padding 或缺失数据混淆。

## 零基础精讲：把这一节慢下来

### 先看一个具体场景

假设词表只有“猫、狗、鱼”，可以给它们三张互不混淆的身份证：猫=[1,0,0]、狗=[0,1,0]、鱼=[0,0,1]。
...
数清输入有几个位置、词表有几个不同词，所以矩阵应有“词元数 × 词表大小”的形状。

运行前先在纸上写出你预计的结果；即使猜错，也要指出自己是在哪个箭头上理解错了。这样比复制代码后看到“能运行”更接近真正学会。

### 本节暂时不要误会

验证集和上线数据不能重新排序词表；否则相同列会突然代表另一个词。

用一句话过关：**给每个词一个唯一列号。看到某词，就创建全零数组，再把它对应的那一列设为 1。**
