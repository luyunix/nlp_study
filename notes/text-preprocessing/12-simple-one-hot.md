...
```text
distance(猫,狗)   = √((1-0)²+(0-1)²) = √2
distance(猫,汽车) = √((1-0)²+(0-0)²+(0-1)²) = √2
```

用余弦相似度时，任意两个不同 One-Hot 的点积都为 0。人知道猫和狗更相似，编码本身却完全表达不出来。One-Hot 只保证身份互不混淆，不承载由语料学到的关系。

### 稀疏到底浪费在哪里

若词表 `V=100,000`，每个词向量有 100,000 个位置却只有一个 1。直接存储稠密数组会保存大量零；矩阵乘法也会做很多没有贡献的计算。

实际框架通常不会真的构造巨大 One-Hot 再乘 Embedding 矩阵，而是拿 ID 直接查对应行：

```text
one_hot(ID) × E  等价于  E[ID]
```

两种写法数学含义相同，查表实现更高效。这也是后面 `nn.Embedding` 接收整数 ID 而不是 One-Hot 的原因。

### One-Hot 什么时候仍是好选择

颜色只有红、绿、蓝，星期只有七类，标签类别数量也很小时，One-Hot 简单、可解释且不会错误引入大小关系。真正的问题不是“方法旧”，而是类别数巨大且希望表达相似度时，One-Hot 的维度和语义能力不合适。

### 去重顺序也是模型契约

用无稳定顺序的方式去重后直接编号，可能让不同运行产生不同列定义。应按首次出现顺序或显式排序建立词表，并把最终映射保存下来。训练、验证和部署必须共用同一份，而不是各自“看起来一样”地重建。
