...
中文无监督 FastText 常把空格当 token 边界。原始句子：

```text
自然语言处理很有趣
```

若不分词，可能被当成一个长 token；按课程路线处理后应类似：

```text
自然 语言 处理 很 有趣
```

FastText 再从这行 token 中构造 CBOW 或 Skip-Gram 窗口。分词错了，后面调 `dim、epoch、lr` 都是在错误单位上学习。

### 文件的五项最低验收

1. **编码**：用 UTF-8 读取无乱码；
2. **边界**：词之间确实有空格，不是看起来像空格的异常字符；
3. **行**：一行是一段可接受的上下文，不把整本书压成一行；
4. **内容**：抽样没有 HTML、导航、重复模板和大量无意义符号；
5. **规模**：统计总行数、总 token 数、不同 token 数与空行数。

大文件无法在 IDE 全部显示，不代表文件缺失。用流式抽样检查开头、中间和末尾，避免一次性读入内存。

### 训练数据和评估数据为什么要分开

无监督词向量不使用人工类别标签，但仍然会“看见”语料中的上下文。若用同一批下游测试文本反复调参数，仍会把测试信息带入选择过程。应固定训练语料、验证任务和最终测试集，尤其在比较不同词向量配置时。

### FastText 与普通 Word2Vec 的额外差别

FastText 的词表示可由词本身及字符 n-gram 子词共同组成，因此能够为很多未登录词组合向量。但这并不取消中文分词：词边界决定训练窗口和词级统计，字符 n-gram 负责在词内部共享形态信息。二者解决的层面不同。
