...
第一步保留原文与标签，不急着删除符号。分词后可能得到：

```text
[手机, 屏幕, 碎了, 申请, 换货]
```

固定词表把它映射成 ID，例如 `[7,31,86,14,52]`；Embedding 再把每个 ID 查成 D 维向量，单句形状从 `[L]=[5]` 变成 `[L,D]=[5,D]`。多个句子组成 batch 时，需要根据训练集长度分布选择最大长度并生成 `[B,L,D]`，不足处用 PAD，超长处按明确规则截断。

### 数据分析怎样反过来决定预处理

若 95% 的评论不超过 40 个 token，极少数日志粘贴有 5000 个 token，直接按最大值补齐会浪费大量计算；可先查异常，再选择覆盖大多数样本的长度。若“售后”只占 5%，只看 Accuracy 会掩盖少数类失败，应同时观察宏平均 F1 与该类召回。

### 每一步都要回答四个问题

| 阶段 | 输入 | 改变了什么 | 输出 | 主要风险 |
|---|---|---|---|---|
| 分词 | 原始字符串 | 确定 token 边界 | token 列表 | 专业词切错 |
| 数字化 | token | 固定身份/查向量 | ID 或 `[L,D]` | 词表不一致 |
| 语料分析 | 全体样本 | 只统计、不改样本 | 分布与异常报告 | 用测试集调规则 |
| 长度规范 | 不等长 ID | 截断、补齐、mask | `[B,L]` | 截掉关键词 |
| 数据增强 | 训练样本 | 生成语义近似表达 | 新训练样本 | 标签漂移 |

预处理不是必须按表中每项都做，而是每个动作都应有数据证据和验证结果。凡是改变文本含义或样本分布的步骤，都要保留原始字段并可回溯。
