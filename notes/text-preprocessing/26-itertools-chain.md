...
外层列表有 3 个元素，每个元素又是一句话的 token 列表。若要统计整个语料的词频，`Counter` 希望依次收到词，而不是依次收到三个小列表。
...
flowchart LR
    ...
    B["[物流, 很快]"]
    C["[退款, 没有, 到账]"]
    A --> D["chain.from_iterable"]
    B --> D
    C --> D
    D --> E["包装 → 破损 → 申请 → 换货 → 物流 → 很快 → …"]
```

`chain.from_iterable(tokenized)` 的意思就是：先把第一句话走完，再接着走第二句话，然后第三句。它没有删除重复词，也没有打乱顺序，只把外层边界去掉。
...
铺平一次后得到的仍是 `["包装"]、["破损"]…` 这些小列表，不会自动一直钻到底。`chain` 不是任意深度递归展开工具。
...
chain(*tokenized)
chain.from_iterable(tokenized)
```

两者在小例子上结果相同。`chain(*tokenized)` 先用星号把外层元素解包成多个位置参数；`chain.from_iterable(tokenized)` 直接接收“列表的列表”。大规模或流式语料更适合后者，因为不必先把所有子列表展开成一次函数调用的参数。

```mermaid
flowchart TB
    A["tokenized：可迭代地提供每句话"] --> B["from_iterable"]
    B --> C["需要时才取下一句话、下一个词"]
    D["*tokenized"] --> E["先把外层各项作为位置参数交出"]
```

### 为什么打印后第二次可能为空

和 `map` 一样，`chain` 返回惰性迭代器：

```python
words = chain.from_iterable(tokenized)
print(list(words))  # 第一次：得到全部词
print(list(words))  # 第二次：[]
```

如果既要统计词频又要多次遍历，有三种选择：

- 数据小：一次转成列表；
- 数据大：在一次遍历中同时完成需要的统计；
- 需要重新读取：重新创建 chain，而不是复用已耗尽对象。

### 最危险的输入是字符串

```python
chain.from_iterable(["包装破损", "物流很快"])
```

会得到单个汉字，因为字符串本身也是字符的可迭代对象。正确前提是每条文本已经被分成 token 列表：

```python
[["包装", "破损"], ["物流", "很快"]]
```

所以本节知识链不是“chain 会分词”，而是“分词已经完成 → chain 去掉句子这一层边界 → 形成全局词流”。
