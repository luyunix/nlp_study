...
模型已经把第 2 列学成“我”、第 3 列学成“爱”。上线新句子 `我 爱 新词` 应得到 `[2,3,1]`，其中 1 是 `<UNK>`。
...
此时 ID 2 已从“我”变成“新词”，但模型参数并不会跟着换含义。代码可以运行，预测却失去任何可信解释，这比直接报错更危险。

### `<UNK>` 与 `<PAD>` 进入模型后的待遇不同

`<UNK>` 代表句子中确实存在一个词，只是词表不知道它是谁，所以它占用真实时间步并拥有可训练向量。`<PAD>` 是为了对齐 batch 补出的空位置，通常要在 attention mask 或损失函数里忽略。
...
若二者共用 ID，模型无法分辨“未知内容”和“根本没有内容”。

### 还要保存哪些东西

只保存 `word→id` 仍可能不够。可复现的输入契约还包括分词器版本、大小写/Unicode 规则、最大长度、截断方向、PAD/UNK ID，以及用户词典。训练和预测任何一项不同，都会改变输入张量。

对序列化格式也要注意安全：只加载可信来源的对象文件。更透明的简单映射可保存为 JSON；若必须使用能执行反序列化逻辑的格式，应限制来源并校验文件。
