...
## 真正看懂这节：词云只把频率画大，不会自动理解情感

目标是比较正负评论中常见的形容词。完整管线是：
...
flowchart LR
    ...
    B --> C["只保留形容词"]
    C --> D["Counter 统计频率"]
    D --> E["WordCloud<br/>频率 → 字体大小"]
    E --> F["回看原句验证"]
```

每一步都可能影响结论。词云只负责最后一步显示，不能补救前面的分词或词性错误。
...
词性工具可能正确筛出“完好、耐心、快”，词云把它们画得很大。如果只看词云，反而会误以为负面用户认为“完好、耐心、快”。原因是词云丢掉了“不、并不”等否定词和词序。
...
因此词云适合回答“哪些词出现得多”，不适合单独回答“用户对什么满意”。

### 频率高也不等于区分度高

“好”可能在正类出现 500 次、负类出现 450 次。它在两个类别都常见，不能很好地区分类别。“退款”可能在负类 100 次、正类 2 次，频率没“好”高，却更有区分度。

比较标签时应至少计算归一化频率：

```text
类内频率 = 某词在该类出现次数 / 该类全部目标词次数
```

否则正类样本比负类多 10 倍时，几乎所有词在正类的原始次数都会更大。

### 词性标注不是标准答案

中文词性取决于分词结果和上下文。“快”可能是形容词，也可能出现在固定名称中；领域新词可能被错误切分。可靠流程是：
...
### WordCloud、Matplotlib 各做什么

- `WordCloud.generate_from_frequencies(counts)`：把词频转换成布局；
- 中文字体路径：决定汉字能否正常显示；
- `matplotlib.pyplot.imshow(...)`：把生成的图像画到画布；
- `axis("off")`：隐藏坐标轴，仅影响显示。

颜色、形状、旋转角度主要是视觉编码，不能当作额外统计结论。
...
flowchart LR
    A["词云<br/>快速浏览"] --> D["综合解释"]
    B["Top-N 频率表<br/>保留精确数字"] --> D
    C["原句样本<br/>保留否定与上下文"] --> D
```

词云可以留，但旁边应同时提供 Top-N 表、类内比例和原句示例。这样既直观，又不会被漂亮图片带偏。
...
词云适合探索和展示，不适合精确比较。字体面积会误导，否定词又可能让“客服不耐心”只剩“耐心”；重要结论要配类内词频表、条形图和原句核查。
