...
评价里同时有“漂亮、简单、很慢”。若想了解描述体验的形容词，可先按词性筛选，再让出现次数决定词云字号。
...
先打印 Counter 确认筛选正确，再画词云；如果计数都不可信，漂亮图片也没有意义。

运行前先在纸上写出你预计的结果；即使猜错，也要指出自己是在哪个箭头上理解错了。这样比复制代码后看到“能运行”更接近真正学会。

### 本节暂时不要误会

词云适合探索和展示，不适合精确比较；重要结论要配表格或条形图。

用一句话过关：**词云把出现次数映射成字体大小。若我们只关心评价用语，可先用词性标注筛出形容词，再统计频率。**
