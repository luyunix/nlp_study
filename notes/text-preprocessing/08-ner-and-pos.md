...
## 真正看懂本节：三个任务输出的数据根本不同

以 `李明在北京大学学习人工智能` 为例：

```text
分词：李明 / 在 / 北京大学 / 学习 / 人工智能
POS：李明=人名性词；在=介词；北京大学=名词；学习=动词……
NER：李明=PER；北京大学=ORG
```

分词输出边界，POS 为每个 token 赋语法角色，NER 输出现实对象的类别和位置。`人工智能` 是名词短语，但不一定是命名实体；“名词”与“实体”不能互相替代。

### NER 为什么还需要 B/I/O

当模型按字符或子词预测时，一个实体可能跨多个位置。常见 BIO 标注把：
...
合并成组织实体“北京大学”。`B` 表示实体开始，`I` 表示继续，`O` 表示不属于实体。POS 的 `n、v、a` 等标签回答语法角色，BIO 的 `PER、LOC、ORG` 回答实体边界与类型，两套标签的目的不同。

### 为什么 `jieba.posseg` 不能等同于完整 NER

`posseg` 的直接职责是分词和词性标注。某些词性代码能提供人名、地名候选，但这不等于经过上下文训练的实体识别器。句子：
...
相同的“苹果”只有结合上下文才可能分别判为公司与普通名词。词典规则可以提供候选，上下文模型才能进一步消歧；最终仍要用标注数据计算实体级 precision、recall 和 F1。

### 评价时不能只看 token 是否命中

若真值是完整实体“北京大学”，模型只识别“北京”，token 可能部分正确，但实体边界是错的。NER 通常需要按完整 span 和类别评估。上线前还要观察哪些人名、机构简称和嵌套实体最常失败，而不是只看一条演示输出。

## 零基础精讲：把这一节慢下来

### 先看一个具体场景

句子“苹果在上海开新店”里，“苹果”可能是机构，“上海”是地点，“开”是动词。分词只画边界，POS 说明语法角色，NER 才尝试指出现实对象。
...
逐行查看 item.word 与 item.flag。先学会查词性标记表，不需要一次背下所有字母代码。

运行前先在纸上写出你预计的结果；即使猜错，也要指出自己是在哪个箭头上理解错了。这样比复制代码后看到“能运行”更接近真正学会。

### 本节暂时不要误会

词性和实体类别都可能预测错；“苹果”离开上下文尤其容易歧义。

用一句话过关：**分词只告诉我们边界；词性标注告诉我们它像名词、动词还是形容词；命名实体识别进一步找人名、地名、机构名等现实对象。**
