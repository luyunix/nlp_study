...
## 真正看懂本节：用户词典是一项可测试的模型配置

假设业务部门名叫 `创新办`，原来被切成 `创新 / 办`。加入用户词典后，目标不是“让这个例句看起来正确”就结束，而是同时检查它会不会伤害别的句子。
...
- `创新办`：候选整体，必填；
- `10000`：影响整体成词倾向的权重，不是必须精确等于真实出现次数；
- `n`：可选词性提示，后面用 POS 过滤时才特别重要。

频率过低可能仍被拆开，过高则可能在不该合并的上下文中强行合并。课程示例中的 10000 只是演示值，不是所有词的推荐常数。

### 用“正例 + 反例”验证，而不是只测一句

```text
正例：创新办主任参加会议        → 希望出现“创新办”
正例：请联系创新办处理          → 希望出现“创新办”
反例：这个方案很有创新办法      → 不希望误把“创新办”跨边界合并
```

每次修改词典都运行同一组回归样句。正例检查该合并的有没有合并，反例检查不该合并的有没有被破坏。用户词典越大，越需要版本号、修改原因和回归数据。

### 文件词典和 `add_word` 怎样选择

`load_userdict` 适合一批稳定、可版本管理的领域词；`add_word` 适合程序启动时动态加入少量配置。无论哪种方式，训练、验证和线上预测必须加载相同词典，否则同一句话会得到不同 token ID，后续模型输入随之改变。

词典只能修正已知边界，不能解决所有上下文歧义。若“苹果”究竟是水果还是公司需要整句判断，应交给 NER 或上下文模型，而不是无限堆词频。
...
公司内部有部门叫“创新办”，通用词典可能切成“创新 / 办”。加入用户词典，相当于告诉分词器：这三个字在本领域更可能是一个整体。
...
代码会在 add_word 前后各打印一次。真正要观察的是边界有没有按预期变化，而不是记住 freq=10000。

运行前先在纸上写出你预计的结果；即使猜错，也要指出自己是在哪个箭头上理解错了。这样比复制代码后看到“能运行”更接近真正学会。

### 本节暂时不要误会

频率在这里是算法权重，不等同于真实出现次数；设得极大可能破坏别的句子。

用一句话过关：**通用词典不认识公司名、产品名或专业术语时，我们可以补一份用户词典，让这些片段更倾向于作为整体出现。**
