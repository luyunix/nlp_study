...
## 真正看懂本节：精确模式到底“精确”在哪里

“精确模式”中的精确，指它尝试给出一条主要且不重叠的切分路径，不表示它知道唯一正确答案。程序面对一句中文时，会遇到许多候选词；最终需要选择一组首尾相接的 token 覆盖原句。

以 `研究生命起源` 为例，可以想象至少两种路径：
...
精确模式会根据词典和统计信息选择一条主要路径。若领域语料中的真实含义与通用词典不同，结果仍可能错，所以“精确”不能替代人工抽样检查或用户词典。

### `cut` 与 `lcut` 只差一个字，数据状态却不同

```python
stream = jieba.cut("自然语言处理", cut_all=False)
words = jieba.lcut("自然语言处理", cut_all=False)
```

`stream` 是生成器：里面的 token 会在遍历时逐个产生。`words` 是已经装好的列表。假设第一次执行：
...
第一行先消费一个 token，第二行只得到**剩余** token。此后再 `list(stream)` 会是空列表。列表则可重复读取，但要一次占用全部结果的内存。小句调试优先 `lcut`；大文件流式处理可保留 `cut`，但要清楚谁消费了它。

### 怎样判断结果适不适合任务

不要只问“分得对不对”，还要问“下游需要什么单位”。做评论词频时，希望一个字符位置只统计一次，精确模式通常合适；做搜索索引时，还希望短查询命中长词，就要在精确结果上补充搜索词。模式由任务选，不由名字的“精确”“全”决定高低。
