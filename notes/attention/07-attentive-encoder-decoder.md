...
Attention context 常写成：

```text
Cₜ = Σᵢ αₜᵢ hᵢ
```

`i` 遍历源序列位置，`t` 表示当前目标解码步。对固定 t，所有 i 的 `αₜᵢ` 经 Softmax 后和为 1；换到下一 t，Decoder 查询变化，整行权重重新计算。

若源长 `Ls=4`、目标生成 `Lt=3`，完整权重矩阵形状是 `[Lt,Ls]=[3,4]`。每一行产生一个 context，每一列对应一个源位置。它不是必须对角对齐，因为翻译可能调序或一词对应多个词。

实现时 Encoder outputs 提供 hᵢ，Decoder 上一步/当前状态形成 Q。context 与 Decoder 表示拼接或融合后才预测词表分布。Attention 是 Encoder 与 Decoder 之间的动态接口，不是独立完成翻译的第三个模型。
