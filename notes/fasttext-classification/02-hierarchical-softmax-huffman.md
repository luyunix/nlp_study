...
## 真正看懂本节：从 C 个类别改成走一条二叉路径

普通 Softmax 要为 C 个输出都算分；层次 Softmax 把类别放在二叉树叶子，预测某类变为从根到叶的一串左右二分类。平衡直觉下路径约 `log₂C` 步，Huffman 树让高频类别路径更短。

叶子概率可理解为路径上每次选择概率的乘积。训练目标仍是提高真实类别路径概率，不是把类别数真的删掉。树结构由频率影响，低频类路径可能更长；是否更快、更好取决于类别规模和数据分布。
