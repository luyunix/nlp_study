...
## 真正看懂本节：保存模型还要保存输入与标签契约

`.bin` 能恢复 FastText 模型参数和词典，但可复现实验还需记录预处理代码、训练数据版本、标签前缀/映射、参数和验证结果。加载后先用固定 smoke test 检查同一文本预测是否一致。

不要只保存“最好的一次”却丢失选择依据；文件名带版本，保留校验哈希和只读来源。加载陌生模型同样属于供应链输入，应确认来源。新版本上线前与旧模型在同一验证集和延迟条件下比较。
