...
## 真正看懂本节：epoch 决定看几遍，lr 决定每步走多远

训练集 10,000 条，batch/内部更新方式确定后，`epoch=5` 表示语料完整遍历五轮，不是五次参数更新。更多轮可能继续拟合，也可能过拟合。

学习率控制 `参数 -= lr×梯度` 的步长。太大可能震荡/发散，太小可能在预算内学不动；FastText 的 lr 通常会随进度变化，传入值是初始设置。调参时固定训练/验证划分，一次只改一项，同时记录指标与耗时。
