...
## 真正看懂本节：自动调参只是自动试验，不会替你定义正确目标

Autotune 在给定时间内尝试多组参数，用指定验证文件和指标选择候选。验证集若与训练重复、标签错误或分布不代表上线，自动搜索只会更高效地过拟合错误目标。

测试集不能交给 autotune；它只在最终配置确定后评估一次。记录时间预算、指标、验证文件版本和最终参数。短预算下结果有随机性，不应把一次搜索输出当全局最优。
