...
## 真正看懂本节：ELMo 让同一个词随句子改变向量

ELMo 使用字符级表示和双向语言模型；目标词最终表示由多层上下文状态加权组合。静态 Word2Vec 中“银行”只有一个向量，ELMo 在“河岸”和“金融机构”语境中会产生不同表示。

它的双向语言模型由左右两个方向建模后组合，不等同于 Transformer 自注意力。ELMo 证明了上下文化表示的价值，是 BERT 前的重要路线；具体速度、迁移方式和效果需结合架构与任务比较，不能只按发布时间排序高低。
