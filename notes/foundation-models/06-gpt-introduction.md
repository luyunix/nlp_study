...
因果 mask 保证位置 t 只能看自己和过去，不能偷看右侧目标。输出 `[B,L,V]`，每个位置预测下一个 token。

生成时给 prompt，取最后位置分布选一个 token，追加后再次前向，循环到停止条件。贪心、采样、温度、top-k/top-p 只改变如何从分布选 token，不改变因果训练主线。

Decoder-only GPT 偏生成；BERT Encoder-only 偏双向理解。二者都使用 Transformer 组件，但可见性和训练目标不同。下一词概率反映语料统计，不自动保证事实、无偏或安全。
