...
## 真正看懂本节：这张图是注意力热力图，不是计算图

评估得到每个目标步对每个源位置的权重，堆叠为：

```text
attention [Lt,Ls]
```

列/x 轴是英语源 token，行/y 轴是法语预测 token；单元格颜色表示该目标步分配给该源位置的权重。行通常近似和为 1，padding 列应被 mask。

绘图前要把矩阵裁到真实生成长度和源长度，否则 PAD/EOS 后空白会误导。token 标签数量必须与矩阵轴长度一致，轴不能互换。

对角线只在近似单调对齐时常见；语序变化、一对多或多对一翻译会出现偏移和扩散。热力图用于诊断，不证明翻译正确，也不是 TensorBoard 展示的模型算子计算图。

保存图时还应记录对应源句、真值、预测译文和 checkpoint，单独一张色块图无法复核。
