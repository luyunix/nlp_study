...
只写“CUDA 12”不够，因为它可能指驱动支持上限、Toolkit 或框架 runtime。也不要硬编码 `cuda:0` 后假设所有机器都有 GPU；使用可检查的 device 逻辑，并让模型、输入、初始状态和标签统一移动。

环境可用的验收是最小计算和项目测试都成功，不是安装过程没有红字。升级任一层后重新跑相同 smoke test；若项目追求确定性，还要记录随机种子和相关确定性设置，但这可能牺牲性能且不能消除所有跨平台差异。
