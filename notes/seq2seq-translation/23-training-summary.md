...
## 真正看懂本节：曲线下降只证明训练目标改善

若每轮平均 NLL 从 3.2 降到 2.1，说明模型更会预测训练/记录数据上的下一个目标 token；它不自动证明译文流畅、事实正确或泛化更好。

必须同时看独立验证 loss、贪心/搜索解码样例和翻译指标，并人工检查专名、否定、数字与 EOS。课程只用约 3000 条演示、训练五轮时，结果主要用于验证流水线，不应包装成充分训练模型。

比较 checkpoint 应使用相同解码策略；Teacher Forcing 下的 loss 与真正自回归生成错误不同。保存最佳验证 checkpoint、最后 checkpoint、优化器状态和词表，才能继续训练或复现实验。

训练耗时也应注明设备、数据量、batch 和参数；一条曲线没有这些上下文不能支持模型优劣结论。
