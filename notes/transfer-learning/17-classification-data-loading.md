...
每条数据应稳定返回 `{"text":字符串,"label":0..C-1}`。加载后检查缺失、重复、类别数量、文本长度和标签映射；划分训练/验证/测试时避免同源或重复文本跨集合。

不要在加载阶段偷偷 tokenize 或根据测试集调整标签。原文与样本 ID要保留，后续错误才能回溯。标签 dtype 最终需 long，映射必须随 checkpoint 保存。
