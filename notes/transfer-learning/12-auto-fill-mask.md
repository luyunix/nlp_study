...
## 真正看懂本节：先定位 mask，再只读该位置词表轴

模型 logits `[B,L,V]`。用 `input_ids==mask_token_id` 找到位置，对该位置 `[V]` 做 top-k，再把候选 ID decode。不能对整个三维 logits 直接 flatten top-k，否则位置与词表混在一起。

替换后的句子用于解释候选；子词候选可能需要正确拼接。top-k 分数仍是上下文补词概率，不是知识库事实。
