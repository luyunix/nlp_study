...
## 真正看懂本节：先按可见性与输出方向选家族

Encoder-only（如 BERT 类）让每个位置双向看上下文，适合理解、分类、NER；Decoder-only（GPT 类）用因果 mask 逐 token 生成；Encoder-Decoder（T5/BART 类）先编码输入再条件生成，适合翻译摘要。

这是架构倾向，不是绝对能力边界。选择还要看 checkpoint 训练目标、语言、许可、长度、大小和任务头，不能只按模型名字或参数量。
