...
因固定 mask 位置和最大长度过滤样本时，应统计被删数量与类别/主题分布，避免只留下特定长度文本。训练 loss 只监督 mask 位置，accuracy 也应只统计目标位置。

复用分类训练循环时不能复用 `[B,C]` 的输出假设；MLM logits 是 `[B,L,V]`。保存 tokenizer、mask 规则和词表版本，否则评估位置含义会改变。
