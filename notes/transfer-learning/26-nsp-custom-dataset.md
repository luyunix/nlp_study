...
## 真正看懂本节：负句太容易会让模型走主题捷径

正样本用真实连续 A/B，负样本把 B 换成随机句。若随机句来自完全不同主题，模型只比较关键词就能分类，不必学习句间连续关系。

构造时防止随机 B 恰好是真下一句、同文档重复或数据跨划分泄漏，并保持正负比例可控。NSP 是经典 BERT 配方之一，不是所有后续模型必须采用。
