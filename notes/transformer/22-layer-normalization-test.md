...
## 真正看懂本节：LayerNorm 要在每个 token 的 D 维内验

输入 `[B,L,D]` 时，LayerNorm 对最后 D 维分别标准化，每个 `(b,l)` token 的均值应约 0、方差约 1。不能把 batch、序列一起求总体均值后宣布测试通过。

常用实现方差取 `unbiased=False`，并含可学习缩放/偏移；初始参数下性质最直观，训练后输出均值/方差会受 affine 参数影响。还应测常数输入、不同 B/L、有限值和梯度。

浮点比较使用容差；epsilon 防止方差为零时除零，因此常数输入不应产生 NaN。
