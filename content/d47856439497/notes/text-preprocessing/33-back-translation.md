...
```text
中文 → 英文：The package is damaged and I want a replacement.
英文 → 中文：包裹有损坏，我想更换商品。
```

新句措辞变了，但仍应属于“负面 / 换货意图”，才可以继承原标签。
...
这些变化能让模型看到同一标签的多种说法，减少只记住固定短语的倾向。
...
否定、程度、数字、实体和条件句最容易在多轮翻译中变化。短文本上下文少，翻译模型可能直接返回原句；这不算有效新增样本，只是重复。
...
先划分训练、验证、测试，再只对训练集回译：

```mermaid
flowchart LR
    A["原始数据"] --> B["先固定 train / valid / test"]
    B --> C["只增强 train"]
    B --> D["valid 保持原始"]
    B --> E["test 保持原始"]
```

如果先回译再随机切分，原句和改写句可能分别落入训练集与测试集。模型测试时见到几乎同义的训练样本，分数会虚高，这属于数据泄漏。
...
3. 用规则或模型检查情感/意图是否一致；
4. 对高风险标签人工抽样；
5. 限制每条原文产生的增强数，避免某些样本被过度复制；
6. 保存原样本 ID、翻译路径和工具版本，便于追溯；
7. 在未增强的验证集上确认真实收益。

自动相似度过高可能意味着只是复制，过低可能意味着语义漂移；它只能做筛选信号，不能单独证明标签正确。

### 隐私和外部服务边界

调用在线翻译或大模型接口会把文本发送到外部服务。真实客服数据可能包含姓名、电话、地址、订单号等敏感信息，必须先脱敏，并确认数据处理权限、服务条款和存储策略。不能为了做数据增强直接上传原始隐私文本。

### 回译不是越多越好

大量低质量回译会放大翻译腔、错误标签和重复模板。正确实验是：
...
三者使用同一原始验证集比较。只有验证指标和错误分析共同改善，才说明增强真正有用。
...
课程中的旧公共翻译接口可能已失效。不要硬编码密钥；还要考虑隐私、费用和服务条款。回译结果必须抽检，且不能让原句与回译句跨到训练集和测试集。
