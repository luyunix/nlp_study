...
| 名称 | 本例结果 | 含义 |
|---|---:|---|
| token 总数 | 6 | 连重复出现也全部计数 |
| type/词汇量 | 5 | 不同词有多少种 |
| “包装”的词频 | 2 | 某个词出现多少次 |

```mermaid
flowchart LR
    A["全局词流<br/>包装, 破损, 包装, …"] --> B["Counter"]
    B --> C1["总 token 数<br/>sum(counts.values()) = 6"]
    B --> C2["词汇量<br/>len(counts) = 5"]
    B --> C3["单词词频<br/>counts['包装'] = 2"]
```

`set(words)` 只能得到不同词集合，适合算词汇量，却丢掉每个词出现几次。`Counter(words)` 同时保留“词 → 次数”。

### 为什么词汇量大不等于语料好

这些 token 都会扩大“不同词”的数量：
...
一个未经清洗的语料可能词汇量惊人，但大量行只出现一次，模型几乎学不到稳定表示，Embedding 表却要为它们占行。
...
它保留至少出现 2 次的词。被删除的词通常映射到 `<UNK>`，而不是从文本中凭空消失。阈值越高：

- 词表更小、模型更省资源；
- 低频噪声减少；
- 也可能误删重要的低频故障词。

所以要查看被过滤词样本和 OOV 比例，不能只追求词表小。

### 训练集、验证集、测试集能不能一起建词表

为了**描述整个数据包**，可以分别统计各集合并做对比。但正式建模时，词表和频率阈值应只由训练集决定：
...
若先看测试集词汇再决定模型词表，测试信息就进入了训练流程。即使没有使用标签，也会让最终评估不再完全独立。
...
词汇统计的价值是暴露分词与清洗质量，为后续词表截断、Embedding 大小和 OOV 策略提供依据。
