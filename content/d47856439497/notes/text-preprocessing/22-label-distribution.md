...
`label` 是模型要预测的答案。这里规定 `1=正向体验`、`0=负向体验`。统计标签分布，就是把 label 列分组计数：
...
柱状图只是把这两个计数画成柱子。它没有训练模型，也没有解释为什么不平衡。

### 为什么训练集和测试集要分别看

假设训练集正负比例为 `90% : 10%`，测试集却是 `50% : 50%`。模型在训练时几乎只见正类，测试时却要处理大量负类，评估会很不稳定。

```mermaid
flowchart TB
    A["训练集标签比例"] --> C["是否与任务现实匹配"]
    B["验证/测试集标签比例"] --> C
    C --> D["检查划分方式、采样偏差与分层切分"]
```
...
若 100 条样本里有 99 条正类，模型永远预测正类：

```text
预测正确 99 条 → Accuracy = 99%
负类召回 0 条   → 关键少数类完全没识别
```

这不是说 Accuracy 永远没用，而是类别不平衡时必须同时看少数类的 Precision、Recall、F1、混淆矩阵，必要时使用宏平均指标。

### 图上差不多高，也不一定平衡

两根柱的视觉高度会受 y 轴范围影响。50 万与 45 万可能接近，500 与 50 则差异巨大。读图时要回到柱子对应的实际数字和比例。

```python
counts = df["label"].value_counts(dropna=False)
ratios = df["label"].value_counts(normalize=True, dropna=False)
print("数量：\n", counts)
print("比例：\n", ratios)
```

`dropna=False` 能把缺失标签也暴露出来，避免统计时悄悄丢掉坏数据。

### 发现不平衡之后，不要立刻乱采样

正确顺序是：

1. 先确认不平衡是真实业务现象，还是采集/划分错误；
2. 再确定哪个类别更重要、误判代价是什么；
3. 建立不改数据的基线；
4. 再比较类别权重、过采样、欠采样或阈值调整；
5. 所有选择只在训练集上实施，验证/测试集保持真实分布。

本节只负责“发现与量化问题”，不负责替你自动决定处理方法。
