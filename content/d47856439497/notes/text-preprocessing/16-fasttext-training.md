...
## 真正看懂本节：一次 `train_unsupervised` 内部发生什么
...
每一对都提供一个“输入词—真实上下文词”的监督信号，所以“无监督”只表示没有人工标签文件，不表示没有正确目标。
...
1. 取出“自然”的词/子词向量；
2. 计算它对候选上下文词的分数；
3. 用真实目标“语言”和采样目标计算 loss；
4. 反向传播求出相关参数梯度；
5. 优化步骤更新“自然”、其字符 n-gram 和输出侧参数。

语料滑动、epoch 重复时，这个过程发生许多次。`epoch=5` 表示完整遍历训练语料五轮，不是只更新五次。

### 日志每一项回答的问题不同

- progress：语料处理到哪里；
- words/sec/thread：吞吐速度，不是质量；
- lr：当前学习率，可能随训练进度衰减；
- loss：当前训练目标的错误规模；
- eta：按当前速度估计剩余时间。

只有语料、模型、loss 形式和参数相同时，loss 曲线才适合直接比较。训练 loss 下降说明模型更会完成训练目标，不自动证明下游效果更好。

### `.bin` 为什么比几行向量更完整

完整模型不仅保存已知词向量，还包含词典、参数配置和 FastText 组合子词向量所需信息。若只导出少数词的向量，就无法一致地查询其他词或 OOV。保存时还应另写实验记录：训练文件版本、命令/参数、包版本和随机性设置，模型文件名本身承载不了这些上下文。
