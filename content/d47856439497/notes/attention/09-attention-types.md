...
- 软注意力对所有位置给连续权重并加权求和，可微、常用；
- 硬注意力选择离散位置或少量位置，普通离散采样不易直接反传。

自注意力描述**Q/K/V 来自哪里**：三者来自同一序列；交叉注意力则 Q 来自一侧、K/V 来自另一侧。自注意力仍通常是软注意力，所以“软”和“自”不是互斥类别。

例如 Transformer Encoder 是软的自注意力；Decoder 还有带因果 mask 的自注意力和对 Encoder memory 的交叉注意力。所谓全局/局部注意力又描述可见范围。

分类时先问：这个术语在回答选择机制、信息来源还是可见范围？不要把所有名称放进一张互斥单选表。
