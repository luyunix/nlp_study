...
## 真正看懂本节：用三个分数手算完整 Attention

假设查询与三个 key 得分为 `[2,1,0]`。Softmax 不是简单除以总和，而是先指数化：

```text
exp → [7.389,2.718,1]
归一化 → [0.665,0.245,0.090]
```

若三个 value 是标量 `[10,20,40]`，context 为：

```text
0.665×10 + 0.245×20 + 0.090×40 = 15.15
```

打分回答“各位置与当前查询多匹配”，Softmax 把不可直接比较的实数变成相对权重，加权求和把多个 value 汇成一个连续表示。

padding 或未来位置必须在 Softmax **之前**把分数设为极小值；若在 Softmax 后直接清零而不重新归一化，剩余权重和不再为 1。数值实现应使用稳定 Softmax，避免手工直接对很大 logits 求 exp。
