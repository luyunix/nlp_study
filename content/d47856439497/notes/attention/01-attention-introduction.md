...
## 真正看懂本节：Attention 不是“挑一个”，而是重新汇总

阅读理解问题是“谁在北京工作”，原文有三个位置表示 `小王 / 北京 / 工作`。模型先给它们分配权重，例如：

```text
[0.10, 0.55, 0.35]
```

权重经 Softmax 后非负且和为 1。context 不是直接复制“北京”，而是：

```text
0.10×V小王 + 0.55×V北京 + 0.35×V工作
```

问题变化为“在哪里工作”时，查询改变，三项权重会重新计算。Attention 的核心是**针对当前任务动态决定如何汇总已有信息**；权重是内部计算信号，不应自动解释为人类因果理由。

若输入 value 为 `[B,L,D]`，权重 `[B,L]`，加权求和会消掉 L，得到 `[B,D]`。后面所有 Q/K/V、bmm 和多头结构都只是把这三步批量化或扩展。
