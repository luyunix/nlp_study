...
## 真正看懂本节：摘要是条件生成，不是截取前几句

Encoder-Decoder 把输入编码后自回归生成摘要 token。`max_length/min_length` 控制生成 token 范围，输入还有独立最大长度；截断输入可能丢掉关键事实。

生成流畅不等于事实一致，需核对数字、实体、否定和来源。长度参数、beam/采样与模型任务前缀应按 checkpoint 说明设置，并用 ROUGE 等指标加人工评估。
