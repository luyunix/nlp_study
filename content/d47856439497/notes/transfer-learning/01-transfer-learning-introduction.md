...
## 真正看懂本节：迁移的是参数起点与语言能力

从零分类要同时学中文规律和业务标签；迁移学习先加载在大语料上学过的 Encoder，再用少量标注调整任务头及部分/全部底座参数。冻结底座只训练头更省数据，全部微调适应性更强但成本与过拟合风险更高。

源任务与目标任务越相关越可能受益，也可能出现负迁移。必须与随机初始化或简单基线在同一验证集比较，不能把“预训练”当效果保证。
