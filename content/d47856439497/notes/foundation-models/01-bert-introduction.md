...
## 真正看懂本节：BERT 学的是每个位置的双向上下文表示

“苹果很好吃”和“苹果发布手机”中的“苹果”会经过同一 token Embedding，但多层双向 Encoder 结合左右上下文后产生不同最终向量。BERT 不是一个固定词向量表的别名。

预训练用大量文本构造 MLM（经典 BERT 还含 NSP）学习通用参数；下游再加分类、NER、问答等任务头微调。BERT 本体偏理解表示，不是默认按左到右自由生成的 Decoder。模型大小、数据和任务适配仍决定效果。
