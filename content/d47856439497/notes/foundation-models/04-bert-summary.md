...
## 真正看懂本节：预训练头会丢，语言底座会留下

经典预训练结束后，MLM/NSP 头不一定用于下游；共享 Encoder 参数才是迁移重点。分类加 `[B,H]→[B,C]` 头，NER 加 `[B,L,H]→[B,L,C]` 头，问答输出起止位置分数。

GLUE/CLUE 是多任务评测集合，不是单一准确率，也不能代表所有业务。比较时要核对具体任务、数据版本、指标和模型规模。BERT 的结论是“自监督预训练能提供可迁移表示”，不是 NSP 永远必要或所有任务都应选 BERT。
