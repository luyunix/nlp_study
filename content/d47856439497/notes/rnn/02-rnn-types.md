...
| 形式 | 例子 | 典型读取方式 |
|---|---|---|
| 多对一 | 一串字符判断姓名国家 | 读完整序列后输出一个类别 |
| 多对多（等长） | 每个 token 做词性/NER | 每个时间步都输出标签 |
| 多对多（不等长） | 翻译、摘要 | Encoder 读输入，Decoder 逐步生成 |
| 一对多 | 给图片生成描述 | 一个条件启动一串输出 |

第二维看循环单元内部结构：Vanilla RNN、LSTM、GRU。一个“多对一任务”既可以用普通 RNN，也可以换成 LSTM；任务形式不会自动决定单元类型。

以 `[B,L,D]=[8,12,32]` 的姓名字符向量为例，多对一分类仍先产生 `[8,12,H]`，再取最终有效状态或池化为 `[8,H]`，映射到国家数 `[8,C]`。多对多标注则保留时间轴，得到 `[8,12,C]`。

“一/多”说的是输入输出元素数量，不是神经网络层数，也不是 batch 大小。实际 batch 中始终可以一次放多条样本。
