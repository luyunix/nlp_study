...
## 真正看懂本节：训练循环相同，状态契约不同

外层的 `zero_grad→forward→loss→backward→step` 不因换成 LSTM 而变化。变化应封装在模型 forward 内：初始化 `(h0,c0)`，调用 LSTM，读取最终 h，再送分类头。

若在批与批之间没有刻意做 stateful 训练，每批状态应重新初始化；把上一批最后状态直接传给下一批，会把互不相关姓名串在一起。若确实跨片段保留状态，还必须在适当边界 detach，避免计算图无限延长。

LSTM 参数更多，训练时间和过拟合行为可能与 RNN 不同。公平比较应固定数据顺序、epoch、优化器策略与评估集，同时报告参数量。不能看到一次曲线更低就断言长期依赖一定被解决。

checkpoint 名称和配置必须标明 LSTM；错误地用 RNN 类加载，即使字段名部分相似也会形状不匹配或语义错误。
