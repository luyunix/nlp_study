...
## 真正看懂本节：Dataset 只定义“一条样本怎样取”

`Dataset.__len__()` 回答有多少条样本，`__getitem__(i)` 把第 i 条原始姓名转成模型所需的 `(x,y)`。它不负责自动组成 batch，也不负责训练。

以 `Abel` 为例，若字符表大小 D=57：

```text
x: [L,D] = [4,57]，dtype=float
y: 标量类别 ID，dtype=long
```

课程实现可能为手写 RNN 增加 batch 轴或采用 `[L,1,D]`；关键是 Dataset 输出约定必须与模型 forward 一致。标签给 `NLLLoss/CrossEntropyLoss` 时应是类别索引，不是 float，也通常不是 One-Hot。

`__getitem__` 应是确定的基础转换：同一索引在无数据增强时返回相同标签与对应姓名。若这里随机改类别、依赖全局可变词表或吞掉异常，会让训练难以复现。

第一次测试 Dataset 要同时检查首条、中间、末条和非法索引；打印原文、类别、张量形状和 dtype，并确认字符维每行恰好有一个有效位置（若采用 One-Hot）。
