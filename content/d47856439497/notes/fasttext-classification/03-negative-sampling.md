...
## 真正看懂本节：只练真实目标和少量“假目标”

对一个正样本，不再计算整个大词表/类别表，而是保留真实目标并抽 k 个负目标，训练模型区分正负。一次更新成本随 k 而不是全部 V 增长。

负样本不是业务中的负面文本，而是本次未出现的候选目标。k 太小信号可能不足，太大计算增加；采样分布会影响哪些常见/稀有目标被看到。训练目标是近似学习表示，负采样分数不能未经校准直接当完整 Softmax 概率。
