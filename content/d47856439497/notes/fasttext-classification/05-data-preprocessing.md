...
## 真正看懂本节：训练和预测必须经过同一个纯函数

原文 `Great-phone!!!` 若训练时变成 `great - phone ! ! !`，预测时也必须完全相同。大小写、标点空格、Unicode 与分词规则任一不同，都会产生训练未见 token。

预处理函数应先在训练集拟定，随后冻结并用于验证/测试；保留 raw_text 便于回溯。不要删除可能改变类别的否定词、数字和实体。用正例、反例及幂等性测试（处理两次结果不再变化）验证规则，而不是只看清洗后“更整齐”。
