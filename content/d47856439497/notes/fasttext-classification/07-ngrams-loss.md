...
## 真正看懂本节：wordNgrams 补局部词序，loss 决定输出训练方式

unigram 看 `不/太/好` 三个独立词；`wordNgrams=2` 还加入 `不_太、太_好`，能区分部分局部语序，但特征数增加、低频组合变多。它不是字符 n-gram，后者在词内部组成子词。

Softmax 适合单标签互斥类别；hierarchical softmax 用树降低大输出计算；OVA 把每类视为独立二分类，适合多标签。loss 不能只按速度选择，必须与标签语义和预测后处理一致。
