...
## 真正看懂本节：多标签不是“类别更多”，而是可以同时为真

文本可同时属于 `体育` 和 `国际`，每类独立产生分数，OVA 用多个二分类目标训练。预测时 `k` 限制最多返回多少标签，`threshold` 过滤低于阈值者；二者共同作用，可能返回少于 k 个甚至空集合。

阈值应在验证集根据 precision/recall 取舍选择，可为每类不同。评估使用 micro/macro F1、样本级指标或业务成本，不能把单标签 accuracy 直接套用。
