...
后半章不是 15 个互不相关的工具，而是一条完整的数据准备路线：

```mermaid
flowchart LR
    A["19–21<br/>词 → ID → 向量"] --> B["22–28<br/>看懂数据分布与词频"]
    B --> C["29–32<br/>构造局部特征与规则张量"]
    C --> D["33<br/>只对训练集做数据增强"]
```

阅读时始终问四个问题：**输入是什么、这一行处理改变了什么、输出的类型/形状是什么、它为下一步解决什么问题。** 每节都使用小型售后文本例子把这四问完整走一遍，再回到老师原声和正式代码。
