...
## 真正看懂本节：环境问题和分词问题是两条线

第一次学习时很容易把两件事混在一起：代码报 `ModuleNotFoundError` 是**运行环境问题**；代码能运行但把专业词切错，是**分词规则或词典问题**。前者要检查 Python 和包，后者才去调整分词器。

### 用一个“装了却找不到”的例子排查环境

假设终端执行 `pip install jieba` 成功，IDE 仍然提示找不到。不要重复安装，先在终端和 IDE 中分别运行：
...
若终端显示 `/envs/nlp/bin/python`，IDE 显示 `/usr/bin/python`，说明两边使用的解释器不同。包被安装到前者，代码却由后者运行。正确修复是让安装命令和运行代码指向同一个解释器，例如：

```bash
python -m pip install jieba
python -c "import sys, jieba; print(sys.executable, jieba.__version__)"
```

`python -m pip` 的价值在于：明确让“这个 python”执行它对应的 pip，减少 `pip` 与 `python` 来自不同环境的歧义。

### 分词改变的不是句子内容，而是下游计算单位
...
字符没有变化，但后续词频、词表 ID、Embedding 查表和模型时间步都会变化。若词表中有 `长江大桥`，方案 A 可以直接查一个向量；方案 B 可能出现未知词 `江大桥`。因此分词不是页面展示的小步骤，而是决定后续张量内容的第一道建模选择。

### “词”和 token 不必相同

传统分词希望 token 接近日常词语；现代子词 tokenizer 可能把生僻词拆成更小单位，以控制词表大小和处理未登录词。不能据此说哪一种绝对正确：搜索、词频统计、序列标注和预训练语言模型需要的单位可能不同。本节先掌握传统中文词分词，后面遇到 BERT 子词时再比较两者。
