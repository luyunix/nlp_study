...
## 真正看懂本节：固定下标 16 是教学约束，不是 MLM 通则

先 tokenize，再确认位置 16 是有效普通 token；把输入该位置替换为 mask，labels 只在该位置保存原 ID，其余为 -100。若句子太短、位置落在特殊 token/PAD，就应过滤或重新选择。

真实 MLM 常随机选择位置并动态遮罩。固定位置便于课堂对齐代码，但会造成位置偏差；训练、评估必须明确到底预测哪个位置。
