...
## 真正看懂本节：抽取式问答是在 context 中选起止位置

模型输出 `start_logits [B,L]` 与 `end_logits [B,L]`，组合出合法 span，再用 offset 映射回原文。答案必须来自给定 context；它不是调用常识自由回答。

问题与长 context 会被截断或滑窗，答案若落在被截部分就无法找到。分数是模型置信信号而非事实保证，还需无答案阈值、最大答案长度和原文核验。
