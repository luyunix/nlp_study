...
## 真正看懂本节：概率是“同一个查询内部”的相对分配

分数 `[100,99,98]` 虽然都很大，Softmax 后仍只看相对差异，约得到 `[0.665,0.245,0.090]`。分数整体都加 100，权重不变；因此 Attention 权重不是绝对置信度。

Softmax 应沿 key/源位置维 L：

```text
scores [B,L] → weights [B,L]
weights.sum(dim=-1) ≈ 1
```

若误沿 batch 维归一化，不同样本会互相竞争，语义错误。Mask 后被屏蔽位置的权重应接近 0，其余有效位置重新归一化。

权重极尖表示 context 接近某个 V，权重平坦表示混合多个 V；两者没有脱离任务的绝对好坏。权重图可帮助诊断，但不能仅凭漂亮对齐证明模型预测正确。
