...
模型用 Q 与每个 K 计算匹配分，再用分数加权对应 V。K 与 V 的位置数必须对齐，因为第 i 个权重必须作用于第 i 个 V；它们的特征维可以不同。

Seq2Seq 交叉注意力中，Q 常来自 Decoder 当前状态，K/V 来自 Encoder 所有位置。自注意力中三者都由同一序列经过不同线性投影得到，因此“来源相同”不代表数值相同。

若 `Q:[B,H]、K:[B,L,H]、V:[B,L,Dv]`，结果为 `[B,Dv]`。Q/K 的最后维需要满足打分规则，输出最后维由 V 决定，而不是由 Q 决定。
