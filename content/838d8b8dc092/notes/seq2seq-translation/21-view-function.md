...
## 真正看懂本节：`view(1,-1)` 只改形状，不改 token

一个标量目标 ID 的 shape 可能是 `[]`，Decoder Embedding 常期望 `[B,L]` 或至少 `[1,1]`。`view(1,-1)` 让第一维为 1，其余元素自动推断：
...
它不会重新编码、复制或改变数值。`-1` 只能出现一次，并要求总元素数保持不变。

张量非连续时 `view` 可能失败，`reshape` 会在必要时创建合适布局；课程场景的切片/标量通常重点是补 batch/sequence 轴。不要用 `squeeze()` 不加维度地删轴，batch size=1 时可能把本应保留的 batch 轴一起删掉。

每次 reshape 前后都写出语义：这个 1 是 batch 还是序列长度，避免“能运行”但 Decoder 把轴理解反了。
