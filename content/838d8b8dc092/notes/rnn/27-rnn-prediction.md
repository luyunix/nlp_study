...
## 真正看懂本节：预测其实是在重放训练时的输入契约

新姓名进入模型前必须使用训练时完全相同的 Unicode 规范化、字符表和字符顺序，得到 `[1,L,57]`。重建模型时 D、H、层数、方向和类别数一致，再加载 state_dict。
...
`topk` 的 ids 仍只是类别编号，必须用训练时保存的 `id_to_category` 解析。这里对 top-k log-probability 逐项 exp 得到对应类别概率；只展示 top-k 时它们之和通常小于 1，因为其他类别概率未显示。

遇到字符表外符号不能让 `find=-1` 静默写入最后一列。应使用训练时定义的未知字符策略或明确拒绝，并记录输入覆盖率。

输出概率不是国籍事实或个人身份结论，只是模型基于训练姓名拼写统计的类别分布。低置信度、类别偏差和训练数据限制都应在使用边界中说明。
