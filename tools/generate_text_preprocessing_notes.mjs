...
const beginnerExamples = {
  5: {
    scene: "你拿到 10 万条商品评论，最危险的做法是马上训练模型。先抽样会发现：有空文本、有重复评论、正负标签比例悬殊，而且句子长度差别很大。文本预处理就是先把这些问题看清，再决定哪些处理真的有用。",
    slow: ["保留一条原始评论和它的标签", "把评论切成模型可处理的单位", "把这些单位映射成数字", "统计标签、长度和词频后再定规则"],
    focus: "先运行代码，确认五个阶段有固定先后。现在不用掌握每个阶段的 API，只要能解释为什么“先分析数据，再截断或增强”。",
    boundary: "预处理不是清洗得越干净越好；任何会改变意思的操作都要谨慎。"
  },
  6: {
    scene: "人看到“南京市长江大桥”会尝试按词义断开，程序只看到连续七个汉字。分成“南京市 / 长江大桥”还是“南京 / 市长 / 江大桥”，会直接影响后面的统计和理解。",
    slow: ["输入一整句连续中文", "分词器查词典并比较可能边界", "选出一条或多条切分路径", "输出可以逐个遍历的词元"],
    focus: "先确认运行代码的 Python 正是安装 jieba 的那个解释器，再观察 list(jieba.cut(...)) 如何把生成器变成可见列表。",
    boundary: "虚拟环境只负责隔离版本；它不会让分词自动变准。"
  },
  7: {
    scene: "做评论词频时，同一个字符位置最好只属于一个主要词，否则会重复计数。精确模式像拿笔在句子上画一组不重叠的分隔线。",
    slow: ["输入“小明硕士毕业于中国科学院计算所”", "精确模式计算候选词的组合", "保留一条主要切分路径", "用斜杠显示每个边界"],
    focus: "先猜 words 是列表还是字符串，再运行。比较 jieba.cut 返回的生成器与 jieba.lcut 直接返回的列表。",
    boundary: "“精确”是模式名称，不代表每个领域词都一定切对；专业词仍可能需要用户词典。"
  },
  8: {
    scene: "如果你在建搜索候选库，宁愿多找到几个可能的词，也不想漏掉用户可能查询的片段。全模式会把“南京”“南京市”“长江”“长江大桥”等重叠候选都列出来。",
    slow: ["从句首开始扫描", "记录所有能成词的片段", "允许候选彼此重叠", "得到高召回但不是唯一切分的列表"],
    focus: "把精确模式和全模式的输出放在一起，圈出只在全模式中出现的词，再看它们是否重叠。",
    boundary: "全模式输出不能直接拼回原句，也不适合需要严格位置顺序的序列模型。"
  },
  9: {
    scene: "商品名是“华为手机保护壳”，用户可能只搜索“手机壳”。搜索模式既保留较长词，又补充短词入口，让部分查询也能找到原文档。",
    slow: ["先做较准确的主切分", "找到其中较长的词", "把长词再补充拆成短片段", "长短片段一起进入搜索索引"],
    focus: "比较 lcut 与 lcut_for_search 两行输出；新增的短词就是额外检索入口，不是另一句新文本。",
    boundary: "搜索模式的目标是少漏搜，不是得到最干净的训练序列。"
  },
  10: {
    scene: "客服数据可能同时出现“软件”和“軟體”。程序能保存这些 Unicode 字符，但词典是否认识地区用语，决定了切分是否合理。",
    slow: ["原样保存繁体文本", "确认读取和写出都使用 UTF-8", "用支持繁体的分词词典匹配", "按业务决定是否另存规范化版本"],
    focus: "先直接分词，不要先转换字形。观察接口是否与简体文本完全相同，再检查结果中的地区词。",
    boundary: "简繁统一会方便统计，但也可能损伤专名或合并原本不同的表达，所以必须保留原文。"
  },
  11: {
    scene: "公司内部有部门叫“创新办”，通用词典可能切成“创新 / 办”。加入用户词典，相当于告诉分词器：这三个字在本领域更可能是一个整体。",
    slow: ["先记录错误切分", "把领域词加入用户词典", "给出适度的频率和可选词性", "重新分词并用真实样本回归检查"],
    focus: "代码会在 add_word 前后各打印一次。真正要观察的是边界有没有按预期变化，而不是记住 freq=10000。",
    boundary: "频率在这里是算法权重，不等同于真实出现次数；设得极大可能破坏别的句子。"
  },
  12: {
    scene: "句子“苹果在上海开新店”里，“苹果”可能是机构，“上海”是地点，“开”是动词。分词只画边界，POS 说明语法角色，NER 才尝试指出现实对象。",
    slow: ["先把句子切成词", "为每个词预测名词、动词等词性", "结合上下文识别人名、地名、机构名", "把实体位置和类别交给下游任务"],
    focus: "逐行查看 item.word 与 item.flag。先学会查词性标记表，不需要一次背下所有字母代码。",
    boundary: "词性和实体类别都可能预测错；“苹果”离开上下文尤其容易歧义。"
  },
  13: {
    scene: "神经网络无法把“猫”加上“狗”，但能处理整数和向量。词表像一本带页码的字典：词 ID 是页码，向量才是从那一页取出的可计算内容。",
    slow: ["对训练文本分词", "建立固定且去重的词表", "把每个词替换为整数 ID", "再把 ID 变成 One-Hot 或稠密向量"],
    focus: "看清同一个“我”为什么两次都得到同一 ID。ID 只是地址，不要对 ID 本身做大小和平均值解释。",
    boundary: "数字化不等于模型已经理解语义；它只是让后续学习成为可能。"
  },
  14: {
    scene: "假设词表只有“猫、狗、鱼”，可以给它们三张互不混淆的身份证：猫=[1,0,0]、狗=[0,1,0]、鱼=[0,0,1]。",
    slow: ["固定词表顺序", "找到当前词的列号", "创建与词表一样长的全零向量", "把对应列改成 1"],
    focus: "数清输入有几个位置、词表有几个不同词，所以矩阵应有“词元数 × 词表大小”的形状。",
    boundary: "验证集和上线数据不能重新排序词表；否则相同列会突然代表另一个词。"
  },
  15: {
    scene: "训练时词表里没有“量子猫”。上线遇到它时，系统不能报错，也不能临时改变所有编号，因此通常把它送到统一的未知词位置 UNK。",
    slow: ["加载训练时保存的词表", "已知词查回原来的 ID", "未知词映射到 UNK", "短句补齐时另用 PAD"],
    focus: "逐个代入 vocab.get：我→1、爱→2、新词→默认值0。先手算 [1,2,0] 再执行。",
    boundary: "UNK 表示“这里有一个不认识的词”，PAD 表示“这里没有内容”，二者不该混为一谈。"
  },
  16: {
    scene: "身份证能区分猫、狗、汽车，却不会告诉你猫和狗比汽车更相似。One-Hot 的优势是不会混淆身份，弱点是完全没有语义距离。",
    slow: ["为每个词分配唯一列", "创建大量为零的向量", "不同词的 1 出现在不同列", "所有不同词之间看起来同样远"],
    focus: "手写循环时盯住 vector[vocab[word]]=1；它只选中一个位置，没有任何训练过程。",
    boundary: "One-Hot 并非错误方法；小型类别特征很好用，只是不适合表达大型词表的语义关系。"
  },
  17: {
    scene: "在“我 爱 自然 语言 处理”中先遮住“自然”，只给模型看“爱”和“语言”。若海量文本反复提供这种猜词题，模型会逐渐学到哪些词常处于相似语境。",
    slow: ["滑动窗口选定中心位置", "把左右词作为输入", "让网络预测整个词表的概率", "用正确中心词计算误差并更新向量"],
    focus: "先只运行样本生成器，不急着训练网络。把每一行读成“根据左、右邻居，应该猜哪个中间词”。",
    boundary: "CBOW 的猜词任务是训练手段；最终需要的是权重里学到的词向量。"
  },
  18: {
    scene: "Skip-Gram 把上一节的箭头反过来：给模型“自然”，分别要求它猜附近可能出现“爱”和“语言”。一个中心词会拆成多道训练题。",
    slow: ["选定一个中心词", "列出窗口内每个邻居", "为每个邻居建立一对样本", "多次误差共同更新中心词向量"],
    focus: "观察同一个 center 为什么连续出现多次。每一行只对应一个上下文标签，而不是一次输出整个句子。",
    boundary: "Skip-Gram 不是 n-gram，也不是跳着切词；它描述的是“中心预测周围”的训练方向。"
  },
  19: {
    scene: "FastText 像学语言的学生，教材若是一整行没有空格的中文，它可能把整句当成一个单位。训练前必须先保证每行文本、空格分词和编码都正确。",
    slow: ["收集与任务匹配的语料", "去掉乱码和明显重复", "中文分词并用空格连接", "以 UTF-8 按行写成训练文件"],
    focus: "打开生成的 wiki_sample.txt，确认你能肉眼看到词间空格和换行；数据格式不对时先别启动训练。",
    boundary: "更多语料不必然更好，领域错位或大量噪声会让模型稳定地学到错误规律。"
  },
  20: {
    scene: "无监督并不是“没有正确答案”。窗口里的真实邻居天然就是答案，模型每猜错一次就微调词向量，遍历大量窗口后形成稳定空间。",
    slow: ["从语料窗口自动造输入和目标", "前向计算预测概率", "用真实邻居计算损失", "反向更新并保存完整模型"],
    focus: "第一次只用极小语料跑通 train→save，不用追求语义质量；日志里的 loss 是训练信号，不是最终业务分数。",
    boundary: "小样本能验证代码通路，却不足以证明学到可靠词义。"
  },
  21: {
    scene: "模型说“语言”的近邻是“文字、语法、中文”，看起来合理，但一个漂亮例子可能只是巧合。评估要用一批固定词和下游任务反复验证。",
    slow: ["加载训练时保存的同一模型", "查询词对应的向量", "用余弦相似度寻找近邻", "再用批量样例或任务指标验证"],
    focus: "先看 vector.shape 是否等于训练维度，再看近邻的 score 和 word；分数只表示空间位置，不表示事实正确。",
    boundary: "不要只挑成功截图；常见词看似合理、低频词一塌糊涂时，整体模型仍可能不可用。"
  },
  22: {
    scene: "调参像调收音机：一次拧五个旋钮，即使声音变清楚也不知道是哪一个起作用。先固定语料和评估集，每次只比较一个关键参数。",
    slow: ["保存一个可复现的基线", "只改变维度、轮次或学习率之一", "记录训练成本和固定评估结果", "根据下游任务而不是直觉选方案"],
    focus: "把参数看成实验条件，不是越大越好。先建立一张“参数—耗时—指标”小表再做决定。",
    boundary: "维度和轮次增加会提高成本，也可能拟合噪声；没有统一的神奇参数。"
  },
  23: {
    scene: "Embedding 像一张“ID→向量”的可训练表；Word2Vec 像一种预先把这张表学好的练习方法。你可以随机建表随任务训练，也可以先装入预训练结果。",
    slow: ["离散词先变成 ID", "Embedding 按 ID 取出一行", "任务损失可以继续更新这行", "Word2Vec 可为这些行提供较好的初始值"],
    focus: "相同 ID 1 出现两次，所以 vectors[0] 与 vectors[2] 相同。先理解查表，再理解梯度如何改表。",
    boundary: "随机初始化的 nn.Embedding 不会天生知道“猫”和“狗”相近。"
  },
  24: {
    scene: "一个批次有 32 句话，每句统一为 20 个词，每个词查成 128 维向量，输出就像 32 个“20×128”的小表，形状是 [32,20,128]。",
    slow: ["句子分词并变成 ID", "把多句 ID 堆成 B×L", "每个 ID 查 Embedding 的一行", "末尾增加向量维得到 B×L×D"],
    focus: "代码输入 [1,3] 表示 1 句话、3 个位置；embedding_dim=4，所以输出必然是 [1,3,4]。",
    boundary: "ID 必须在 0 到词表行数减 1 之间；越界不是未知词策略，而是程序错误。"
  },
  25: {
    scene: "300 维词向量无法直接画在屏幕上，降维就像把立体城市拍成一张照片：能看大致街区，但远近关系会因拍摄角度而变形。",
    slow: ["收集同一空间中的高维向量", "用 PCA 或 t-SNE 投影到 2D/3D", "用词标签观察簇和离群点", "回原空间计算指标验证猜想"],
    focus: "先让 TensorBoard 成功读到 runs 目录，再在 Projector 搜索三个示例词；图用于发现问题，不用于单独下结论。",
    boundary: "二维图上靠近不保证原高维空间最近，投影结果也会受算法参数影响。"
  },
  26: {
    scene: "1000 条样本中 950 条是正面，模型永远猜正面也有 95% 准确率。这个数字很好看，却一次负面都找不到。",
    slow: ["分别统计每个标签数量", "换算比例并检查数据划分", "用混淆矩阵查看各类错法", "必要时使用权重、重采样和更合适指标"],
    focus: "先手算 Counter 结果，再区分“数量”和“比例”。不要看到不平衡就立即删除多数类。",
    boundary: "准确率在类别不平衡时可能严重误导，需同时看 precision、recall 和 F1。"
  },
  27: {
    scene: "你有一万句话，要对每句执行同一个 len。map 像传送带：元素走到工位时才计算，结果被取走后不会自动倒带。",
    slow: ["准备可迭代的句子", "把同一个函数交给 map", "遍历时逐项计算", "消费结束后若要重用就重新创建"],
    focus: "预测两次 list(lengths) 的差别。第一次把传送带取空，第二次因此得到空列表。",
    boundary: "惰性迭代器节省内存，但不适合在不知情时反复遍历。"
  },
  28: {
    scene: "99% 的评论不超过 80 个 token，只有一条粘贴了整篇说明书、长度 5000。若按最大值补齐，每条普通评论都会浪费大量空位置。",
    slow: ["用模型实际 tokenizer 计算每条长度", "画直方图并查看 95/99 分位", "抽查长尾样本是否异常", "结合显存和验证效果选择目标长度"],
    focus: "先比较中位数、95 分位和最大值，理解为什么一个 100 不应强迫其他数据全部补到 100。",
    boundary: "字符数、jieba 词数和模型 token 数不是同一个长度单位。"
  },
  29: {
    scene: "如果正面样本全来自长评论、负面样本全来自短标题，模型可能只看长度就猜标签，根本没有理解情感。",
    slow: ["按标签分别收集文本长度", "比较中位数、分布和离群点", "回看差异最大的原始样本", "判断是真实规律还是采集泄漏"],
    focus: "代码中 80 会拉高正类平均值。删不删不能只看数字，要先打开这条原文查原因。",
    boundary: "长度差异本身不一定有问题；危险的是它来自数据来源而非任务语义。"
  },
  30: {
    scene: "分词后得到 [[我,爱,NLP],[我,学,分词]]。做全局词频前要把两层列表铺成一个连续词流，但没必要反复复制整批数据。",
    slow: ["每句话各自得到词列表", "chain 依次接上这些列表", "遍历时才产出下一个词", "直接交给 Counter 做全局计数"],
    focus: "和 map 一样，先预测为什么第二次 list(words) 为空；它也是一次性惰性迭代器。",
    boundary: "若把一个字符串误传给 chain，它会逐字符展开，因此要先确认嵌套层级。"
  },
  31: {
    scene: "“我 爱 NLP 我”总词数是 4，但不同词只有 3 个。前者决定处理了多少位置，后者决定词表和 Embedding 大约需要多少行。",
    slow: ["把训练语料铺成词流", "Counter 累加每个词出现次数", "len(counts) 得到不同词数量", "按最低频率过滤并加入 PAD、UNK"],
    focus: "先手算示例里的总词数、词汇量和最高频词，再与程序输出核对。",
    boundary: "严格实验只用训练集建词表；提前看测试集会产生数据泄漏。"
  },
  32: {
    scene: "评价里同时有“漂亮、简单、很慢”。若想了解描述体验的形容词，可先按词性筛选，再让出现次数决定词云字号。",
    slow: ["逐条评论分词并标词性", "只保留目标词性", "Counter 统计出现次数", "词云把频率映射成字体大小"],
    focus: "先打印 Counter 确认筛选正确，再画词云；如果计数都不可信，漂亮图片也没有意义。",
    boundary: "词云适合探索和展示，不适合精确比较；重要结论要配表格或条形图。"
  },
  33: {
    scene: "zip 像拉拉链：words 的第一个元素和 labels 的第一个元素扣在一起。若一边更短，普通 zip 会在短边结束时悄悄停下。",
    slow: ["准备两个或多个序列", "取各序列同一位置的元素", "组成一个元组", "任一输入耗尽时默认停止"],
    focus: "示例有 3 个词却只有 2 个标签，所以只会输出 2 对。用 strict=True 可以让这种错误直接报出来。",
    boundary: "zip 不会自动补空值，默认静默截短可能让训练数据错位。"
  },
  34: {
    scene: "“电影 好看”和“电影 不 好看”都含有“电影、好看”。只数孤立词容易忽略否定；bigram 能保留“不→好看”这段局部顺序。",
    slow: ["选择词或字符作为基本单位", "放置长度为 n 的连续窗口", "窗口每次右移一格", "把每个局部片段作为新特征"],
    focus: "4 个 token、窗口 n=2 会产生 4-2+1=3 个 bigram。先用公式手算数量再看列表。",
    boundary: "n 越大特征越稀疏；n-gram 只保留局部顺序，不能完整理解长距离语义。"
  },
  35: {
    scene: "比较两份购物清单时，必须先约定每一列都代表同一种商品。文本也一样：两个向量的第 0 列若代表不同词，算出的距离没有意义。",
    slow: ["两段文本用同样规则提取特征", "合并并固定共同词表", "得到列含义一致的两个向量", "再选择余弦、欧氏或 Jaccard"],
    focus: "示例两向量前两列都代表“机器、学习”。先确认列对齐，再把 0.707 理解为方向相似程度。",
    boundary: "相似度高只说明当前表示和度量下接近，不保证两句话事实相同或可以互相替换。"
  },
  36: {
    scene: "GPU 要把多句话堆成一个规则长方体。短句像短木板，要补 PAD；长句像超长木板，要截掉一端，才能统一成相同长度。",
    slow: ["从长度分布选择目标 L", "超过 L 的序列按任务决定截哪端", "不足 L 的序列补独立 PAD ID", "模型用 mask 忽略补齐位置"],
    focus: "第一行短序列右侧补三个 0；第二行从左截掉 1。先在纸上写齐目标长度再运行。",
    boundary: "只补 PAD 不做 mask，会让模型把“空位置”当成真实语言学习。"
  },
  37: {
    scene: "“这家店服务很好”先译成英文再译回中文，可能变成“这家商店的服务非常棒”。表达变化了，正面标签大致没变，于是多出一条训练样本。",
    slow: ["保留原文和原标签", "翻到中间语言", "再翻回原语言得到候选改写", "检查去重、实体、否定和标签后才加入数据"],
    focus: "示例代码没有调用翻译服务，它只展示合格输入输出的样子。真实系统还要记录来源、费用和失败重试。",
    boundary: "回译可能改变否定范围、数字、人名或立场；不经质量过滤就沿用旧标签会制造脏数据。"
  }
};

function makeBeginnerGuide(lesson) {
  const example = beginnerExamples[lesson.p];
  if (!example) return "";
  const slowSteps = example.slow.map((step, index) => `${index + 1}. ${step}`).join("\n");
  return `## 零基础精讲：把这一节慢下来
