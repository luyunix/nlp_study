...
const lessons = [
  ...
  },
  {
    ...
    article: article([
      ["0:00–9:46", "先把注意力放回前向传播：它是加权环节，不是整个模型", `老师先区分软注意力和硬注意力。软注意力不会只留下一个位置，而是给多个位置连续权重，例如 0.5、0.3、0.1；硬注意力更接近只选择极少数位置。课程后面还会讲自注意力，但本节先把最通用的计算骨架讲清楚。
...
这里最重要的边界是：注意力机制只是前向传播中的一个环节，不等于一套能独立完成所有任务的模型。它接收查询、索引和内容，完成一次动态加权汇总，再把增强后的表示交给后续网络。把这层边界立住，后面看到 Attention、Self-Attention 或 Multi-Head Attention 时才不会把不同层次混在一起。`],
      ["9:47–16:42", "第一步：让 Q 与所有 K 比较，得到权重分布", `老师把计算概括成两句话：先算权重，再做加权求和。第一步不能只拿 Q 和某一个 K 比，而是让当前问题 Q 与所有索引 K 逐一计算相关程度，得到一组分数。相似度可以用点积、余弦相似度等方式，具体公式可以变化，但“一个查询对应所有候选位置的一组分数”不变。
...
原始相关性分数还不一定是概率。实际实现通常再用 Softmax 把它变成非负且总和为 1 的 attention weights；归一化必须沿候选位置维完成，不能误写到 batch 或特征维。`],
      ["16:42–23:39", "第二步：用权重乘对应 V，再把结果相加", `拿到权重后，老师把它解释成每份内容的重要程度系数。每个权重乘同一位置的 V 向量，再把所有乘积相加，得到本次注意力的输出。专业表达就是“attention weights 与对应 value 向量做加权求和”。
...
老师把这个输出称为 attention output，后续架构中也常称 context。名字不同，作用相同：它把分散在多个位置的信息压成一份与当前查询相关的表示。`],
      ["23:39–30:29", "为什么说输出是“增强后的 Q”", `老师随后解释注意力真正改变了什么。过去若只把 eat 自己的词向量送给后续网络，模型能利用的上下文有限；注意力输出则融合了整句中与 eat 最相关的信息。因此它不再是孤立的单词向量，而可以理解为一份上下文增强表示。
...
老师反复强调“普通 Q 升级成更强大的 Q”，指的正是这一步：查询本身提出需求，K 决定如何匹配，V 提供真正内容，最终输出综合了全句最相关的信息。`],
      ["30:29–37:25", "回到翻译与长上下文：权重会随当前任务动态变化", `课程最后把注意力与 RNN 对比。循环网络按时间顺序传递状态，长距离信息可能逐步衰减；注意力允许当前步骤直接对所有候选位置重新分配权重。它不能自动消除全部计算成本，也不是“其他位置完全不看”，但能让模型把更多容量放到当前最相关的信息上。
